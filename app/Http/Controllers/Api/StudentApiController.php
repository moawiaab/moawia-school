...
use App\Http\Resources\Admin\StudentDetailsResource;
use App\Http\Resources\Admin\StudentResource;
use App\Http\Resources\Admin\StudentTreeDetailsResource;
...
class StudentApiController extends Controller
{
    ...
    public function store(StoreStudentRequest $request)
    {
        ...
        $student = Student::create($request->validated() + ['account_id' => 1, 'user_id' => $user->id]);
    ...
    public function show(Student $student)
    {
        ...
        return response([
            'data' => new StudentDetailsResource($student),
            'meta' => [
                // 'trees'  => new StudentTreeDetailsResource($student->level[0]->level),
                // 'materials' => $student->level,
            ]
        ]);
    }

    public function showData(Student $student)
    {
        return response([
            'data' => new StudentTreeDetailsResource($student),
            'meta' => [
                // 'trees'  => new StudentTreeDetailsResource($student),
                // 'materials' => $student->level,
            ]
        ]);
    }
